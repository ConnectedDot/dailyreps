...
function WorkoutHistoryList({date}: Props) {
	...
	const workouts = useQuery(api.functions.workouts.listWithRepsForHistory, {
		start: start.getTime(),
		end: end.getTime(),
	});

	function onRepEntryPressed(entryId: string) {
		// router.push(`/(screens)edit-entry/${entryId}`)
		router.push({
			pathname: "/(screens)/log-reps/[workoutId]",
			params: {workoutId: entryId},
		});
	}
		...
		<>
			{workouts.map(wo => (
				<ThemedView key={wo._id} style={styles.stepContainer}>
					...
					{!wo.loggedRepEntries ?
						<ThemedText> No reps logged. </ThemedText>
					:	<View style={styles.entryListWrapper}>
							{wo.loggedRepEntries.map((lre: any) => (
								<ListItem
									key={lre._id}
									onPress={() => onRepEntryPressed(lre._id)}
								>
									<Text>
										{lre.reps} at {new Date(lre.timestamp).toLocaleTimeString()}
									</Text>
								</ListItem>
							))}
						</View>
